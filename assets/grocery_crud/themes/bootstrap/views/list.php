<table border="0" class="display groceryCrudTable table table-striped" id="<?php echo uniqid(); ?>">
	<thead>
		<tr>
			<?php foreach($columns as $column){?>
				<th class=""><?php echo $column->display_as; ?></th>
			<?php }?>
	...
	<tbody>
		...
		<tr id='row-<?php echo $num_row?>'>
			...
			<td class='actions'>
				...
				?>
						<a href="<?php echo $action_url; ?>" class="edit_button ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary btn btn-primary" role="button">
				...
				?>
				<?php if(!$unset_read){?>
					<a href="<?php echo $row->read_url?>" class="edit_button ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary btn btn-info" role="button">
						<span class="ui-button-icon-primary ui-icon ui-icon-document glyphicon glyphicon-eye-open"></span>
						<span class="ui-button-text">&nbsp;<?php echo $this->l('list_view'); ?></span>
					</a>
				<?php }?>

				<?php if(!$unset_edit){?>
					<a href="<?php echo $row->edit_url?>" class="edit_button ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary btn btn-success" role="button">
						<span class="ui-button-icon-primary ui-icon ui-icon-pencil glyphicon glyphicon-pencil "></span>
						<span class="ui-button-text">&nbsp;<?php echo $this->l('list_edit'); ?></span>
					</a>
				<?php }?>
				<?php if(!$unset_delete && $_SESSION['member-id'][0]->admin_utilisateur){?>
					<a onclick = "javascript: return delete_row('<?php echo $row->delete_url?>', '<?php echo $num_row?>')"
						href="javascript:void(0)" class="delete_button ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary btn btn-danger" role="button">
						<span class="ui-button-icon-primary ui-icon ui-icon-circle-minus glyphicon glyphicon-trash"></span>
