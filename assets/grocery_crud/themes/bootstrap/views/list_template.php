<?php
    ...
    $this->set_css($this->default_theme_path.'/bootstrap/css/bootstrap-flex.min.css');
    $this->set_css($this->default_theme_path.'/bootstrap/css/perso.css');
	...
	$this->set_js_lib($this->default_theme_path.'/bootstrap/js/jquery.dataTables.min.js');
	$this->set_js($this->default_theme_path.'/bootstrap/js/datatables-extras.js');
	$this->set_js($this->default_theme_path.'/bootstrap/js/datatables.js');
	$this->set_js($this->default_theme_path.'/bootstrap/extras/TableTools/media/js/ZeroClipboard.js');
	$this->set_js($this->default_theme_path.'/bootstrap/extras/TableTools/media/js/TableTools.min.js');
...
<div class="grocerycrud-container">
	...
	<div class="dataTablesContainer table-responsive">
        <h3>Liste des <?php echo $subject ?></h3>
		<?php if(!$unset_add){
                if(!$_SESSION['member-id'][0]->admin_utilisateur){
                    if($subject != "Profils utilisateur"){ ?>
                    <div class="datatables-add-button">
                        <a role="button" class="add_button ui-button ui-widget ui-state-default ui-corner-all ui-button-text-icon-primary btn btn-primary" href="<?php echo $add_url?>">
